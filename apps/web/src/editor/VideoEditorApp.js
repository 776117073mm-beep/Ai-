...
export function VideoEditorApp() {
    ...
    async function addAssetToTimeline(assetId) {
        ...
        try {
            ...
            const payload = await response.json();
            if (!response.ok)
                throw new Error(payload.error || 'Could not add clip');
